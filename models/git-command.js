class GitCommand {
	...
	status() {
        ...
        */
		...
		let change = Object.keys(this.working_directory.new_changes); //Get the object keys of changes if therese any
		if (change.length > 0) {
			change = `You have ${change.length} change/s.\n${change[0]}\n${change[1]}`; //Output a description on how many and what file(s) are changed
		} else {
			change = `You have 0 change/s.\n`;
		}
		return change;
	}
